...
## com.qlangtech.tis.plugins.incr.flink.connector.hudi.scripttype.ScriptType
...
1. 主键
	...
	* **说明:** 无
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@7051777c
...
1. 主键
	...
	* **说明:** 无
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@2b97cc1f
	...
	* **说明:** 
		...
		2. Standalone: [详细请查看](https://nightlies.apache.org/flink/flink-docs-release-1.14/docs/deployment/resource-providers/standalone/overview/)
		   ...
		   [安装说明](http://tis.pub/docs/install/flink-cluster/standalone/):
		      - 下载、解压
		        ```shell script
		         wget http://tis-release.oss-cn-beijing.aliyuncs.com/3.6.0-alpha/tis/flink-tis-1.13.1-bin.tar.gz && rm -rf flink-tis-1.13.1 && mkdir flink-tis-1.13.1 && tar xvf flink-tis-1.13.1-bin.tar.gz -C ./flink-tis-1.13.1
		        ```
		      - 启动Flink-Cluster：
		         ```shell script
...
4. index
	...
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@226b143b
...
4. column
	...
	* **说明:** 
		 ...
		 * 类型转换
		 ...
		 | Boolean  | boolean |
		 | Bytes    | bytes |
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@1d12b024
...
5. collectionName
	...
	* **说明:** MonogoDB的集合名。【必填】
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@72fe8a4f
...
3. uniqueKey
	...
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@5f13be1
...
2. uniqueKey
	...
	* **说明:** 
		...
		* 注意：如果此参数为空，并且写入模式为 update 和 replace 时，应用会自动获取数据库中的唯一索引；
		如果数据表没有唯一索引，但是写入模式配置为 update 和 replace，应用会以 insert 的方式写入数据；
	* **默认值:** com.qlangtech.tis.trigger.util.JsonUtil$UnCacheString@3be4f71
